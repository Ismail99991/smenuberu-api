...
async function geoRoutes(app) {
    ...
    app.get("/geocode", async (req, reply) => {
        const { address } = zod_1.z.object({ address: zod_1.z.string().min(3) }).parse(req.query);
        ...
        const data = await r.json().catch(() => null);
        const member = data?.response?.GeoObjectCollection?.featureMember?.[0]?.GeoObject ?? null;
        const posStr = member?.Point?.pos ?? null;
        const textAddress = member?.metaDataProperty?.GeocoderMetaData?.text ?? undefined;
