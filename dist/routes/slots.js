...
function fmtISODateUTC(dt) {
    return `${dt.getUTCFullYear()}-${pad2(dt.getUTCMonth() + 1)}-${pad2(dt.getUTCDate())}`;
}
function fmtTimeUTC(dt) {
...
async function slotsRoutes(app) {
    /**
     * GET /slots
    ...
    app.get("/:id", async (req, reply) => {
        ...
        try {
            ...
            const userBooking = slot.bookings.find((b) => b.userId === userId);
            if (userBooking) {
    ...
    app.patch("/:id", async (req, reply) => {
        ...
        if (body.startTime !== undefined) {
            ...
            updateData.startTime = new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), sh, sm, 0));
        }
        if (body.endTime !== undefined) {
            const [eh, em] = String(body.endTime).split(":").map(Number);
            const existing = await prisma.slot.findUnique({ where: { id: slotId }, select: { date: true } });
            const date = existing?.date || new Date();
            updateData.endTime = new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), eh, em, 0));
        }
        if (body.pay !== undefined)
            updateData.pay = Math.round(Number(body.pay));
        if (body.type !== undefined)
            updateData.type = body.type;
        if (body.hot !== undefined)
            updateData.hot = Boolean(body.hot);
    ...
    app.post("/:id/start", async (req, reply) => {
        ...
        if (!slot)
            return reply.code(404).send({ ok: false, error: "slot not found" });
        const booking = (slot.bookings ?? []).find((b) => b.userId === userId && b.status === "booked");
        if (!booking)
